    ...
    while (Serial1.available() > 0) {
        ...
        if (bufferIndex >= 24) {
            ...
            if (rxBuffer[0] == '<' && rxBuffer[23] == '>') {
                ...
                // -----------------------------------------------------------
                // 🛠️ 絕對陣列手動解包 (24 Bytes 精準拆解)
                // -----------------------------------------------------------
                // 💡 由於增加了 1 Byte padding，analogSignals 起始點位移到 rxBuffer[2]
                // A1: rxBuffer[2](低) + rxBuffer[3](高)
                // A2: rxBuffer[4](低) + rxBuffer[5](高) ... 依此類推
                for (int i = 0; i < 6; i++) {
                    int baseIdx = 2 + (i * 2);
                ...
                joyX_raw = rxBuffer[2] | (rxBuffer[3] << 8);
                joyY_raw = rxBuffer[4] | (rxBuffer[5] << 8);
                
                // 3. 拆解按鈕狀態 btnState[8] (從第 14 個 Byte 開始，rxBuffer[14] 是 btnState[0])
                currentClawBtnState = rxBuffer[14];
