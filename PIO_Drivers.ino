...
void setupPioServo(PIO pio, uint sm, uint pin) {
    // 建立陣列，分別記錄 pio0, pio1, pio2 的載入狀態
    static uint offset_servo[3];
    static bool programLoaded_servo[3] = {false, false, false};
    
    uint pio_idx = pio_get_index(pio); // 取得當前是哪一個 PIO 硬體 (0, 1, 2)
    ...
    sm_config_set_wrap(&c, offset_servo[pio_idx], offset_servo[pio_idx] + 6);
    sm_config_set_set_pins(&c, pin, 1);
    sm_config_set_clkdiv(&c, (float)clock_get_hz(clk_sys) / 2000000.0f);
    pio_sm_init(pio, sm, offset_servo[pio_idx], &c);
...
void setupPioMotorPWM(PIO pio, uint sm, uint pin) {
    ...
    pio_sm_set_consecutive_pindirs(pio, sm, pin, 1, true);
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset_pwm[pio_idx], offset_pwm[pio_idx] + 6);
    sm_config_set_set_pins(&c, pin, 1);
    sm_config_set_clkdiv(&c, (float)clock_get_hz(clk_sys) / 2000000.0f);
    pio_sm_init(pio, sm, offset_pwm[pio_idx], &c);
    pio_sm_set_enabled(pio, sm, true);
